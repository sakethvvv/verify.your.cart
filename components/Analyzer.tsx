...
import { AlertTriangle, CheckCircle, ShieldCheck, Shield, Zap, Lock, Globe, Database, Lightbulb, Sparkles, Star, BrainCircuit, DollarSign, Store, FileText, ExternalLink, Info, Settings, Key } from 'lucide-react';
...
export const Analyzer: React.FC = () => {
  ...
  // Detect if a system key is present (for deployment UX)
  const hasSystemKey = process.env.API_KEY && process.env.API_KEY !== "PLACEHOLDER_API_KEY" && process.env.API_KEY !== "";
  const isKeyConfigured = hasSystemKey || customApiKey;

  const resultRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    // If no key is configured on load, gently prompt the user by opening settings
    if (!isKeyConfigured) {
        // Small delay to let animations finish
        const timer = setTimeout(() => setShowSettings(true), 1500);
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    try {
      // Pass the custom key if set, otherwise the service will try process.env
      const data = await analyzeProduct(processedUrl, customApiKey);
      setResult(data);
    } catch (err) {
          ...
          <div className="w-full max-w-2xl mx-auto mt-6 md:mt-8 animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
            <form onSubmit={handleAnalyze} className="relative group z-20">
              ...
              <div className="relative flex items-center bg-white rounded-2xl shadow-2xl p-2 md:p-2.5 transition-all duration-300 transform group-hover:scale-[1.01] focus-within:ring-4 focus-within:ring-blue-500/20 focus-within:border-blue-500 focus-within:scale-[1.01]">
                ...
                {/* Settings Toggle Button with Alert Badge if no key */}
                <button
                    type="button"
                    onClick={() => setShowSettings(!showSettings)}
                    className={`mr-2 p-2 rounded-xl transition-colors relative ${showSettings || customApiKey ? 'text-blue-600 bg-blue-50' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100'}`}
              ...
              {(showSettings || (!isKeyConfigured && !loading)) && (
                  <div className={`absolute top-full left-0 right-0 mt-3 p-4 bg-white/95 backdrop-blur-md border rounded-2xl shadow-xl z-30 animate-enter-smooth ${!isKeyConfigured ? 'border-amber-200 ring-4 ring-amber-50' : 'border-slate-200'}`}>
                      <div className="flex flex-col gap-2">
                          <label className="text-xs font-bold text-slate-500 uppercase tracking-wide flex items-center gap-2 justify-between">
                              <span className="flex items-center gap-2"><Key size={14} /> Custom Gemini API Key</span>
                              {!isKeyConfigured && <span className="text-amber-600 text-[10px] flex items-center gap-1"><AlertTriangle size={10} /> Setup Required</span>}
                          </label>
                          <input 
                            type="password"
                            placeholder="Paste your Google Gemini API Key here..."
                            className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"
                            value={customApiKey}
                          ...
                          <div className="flex justify-between items-center mt-1">
                              ...
                              {!hasSystemKey && (
                                  <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" className="text-[10px] font-bold text-blue-600 hover:underline flex items-center gap-1">
